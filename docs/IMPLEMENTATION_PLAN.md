...
| R3 | `1`, `3` または `b3` | すべて。**ただし sus7 は除く**（sus は3度を持たないコードなので、代わりに `1`, `11` の R4 を使う。ここに3度を入れると sus の意味が消えて4度とぶつかる） |
| R10 | `1`, `3` または `b3`（+1オクターブ） | すべて。**10度になるので幅16半音**。片手の幅上限ちょうど。sus7 には作らない |
| R6 | `1`, `6` | major, major7 **のみ**。~~minor7~~ は誤り（[FEEDBACK_01.md §4](FEEDBACK_01.md)）。Dm7 に R6 を当てると `D+B` になり、コードを決める b7 が消えて Dm6 の響きになる |
| 3音版 | `1`, `b7`, `3` / `1`, `7`, `3` / `1`, `b7`, `10` | shell3 プリセットで使う |

**dim7 は** `1`, `b3` と `1`, `bb7` を使う。

**Powell の選好重み（[FEEDBACK_01.md §2](FEEDBACK_01.md) で追加）**:

利用者から「全然バドっぽくない。7度と10度をめっちゃ使ってほしい」との指摘。以下を守ること。

- **Powell / shell3 では `score.ts` の `varietyPenalty` を無効にする**（重み0）。同じ型の反復は Powell の様式そのものであり、汎用スコアの「同じ family が続くと減点」は逆効果
- `POWELL_FAMILY_BIAS` を `constants.ts` に持ち、R7 と R10 を優先、R3 と R6 を控えめにする
- R3（近接の3度）は低い位置では Powell が避けた響きなので、多用しない
...
| 中間の拍 | **強拍（3拍目）はコードトーン**、弱拍はスケール音も可。直前から±4半音以内に候補を絞り、近い音ほど急峻に優先する。~~「2半音以内を優先」~~ という書き方では優先度が弱すぎて線が飛び回った（[FEEDBACK_01.md §1](FEEDBACK_01.md)） |
| 次のコードの直前の拍 | **アプローチ音**。次のルートの半音下(0.3)・半音上(0.2)・スケール上の隣接音(0.3)・次ルートの完全5度上(0.2) から抽選。**半音アプローチだけに寄せない**。全部半音にすると歩き方が単調になり、5度圏で降りてくる動きが出なくなる |

**追加ルール**:
- 連続する2音の跳躍は原則5半音以内。コードの頭でのみ例外を許す
- 音域の中心から離れるほど、中心方向へ戻る候補を優先する（1コーラスで使う幅は1.5オクターブ以内を目安）
- コーラスの1小節目の1拍目は必ずルート
- 1小節に2コードある場合、アプローチ音を毎回入れない（確率 0.6 程度。残りはコードトーン）
...
- コーラス数 N のときは、**コーラスごとにテイクとリズムを引き直して**連結する。`buildPerformance` は `takeForChorus?: (chorusIndex: number) => Take` を受け取り、2周目以降はこれでボイシングを引き直す（1周目は画面に出ているテイクと一致させる）。**ここを省くと全コーラスで同じ響きが繰り返され、このアプリの主目的が消える**
...
| `perform.test.ts` | 複数コーラスでボイシングが引き直される。1周目は画面のテイクと一致する |

第2段階では以下も追加する（[FEEDBACK_01.md](FEEDBACK_01.md)）。

| ファイル | 検証内容 |
| --- | --- |
| `powell.test.ts` | Dm7 に6度が入らない。R7 と R10 の出現率が R3・R6 より高い |
| `loop.test.ts` | 長時間のループで次コーラスの開始時刻が現在時刻を下回らない（§3 のバグの回帰テスト） |
